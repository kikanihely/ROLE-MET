...
const langFlowGetScore = async (data) => {
  try {
    ...
    console.log("🔥 Raw Langflow Output:\n", langflowOutput);

    // Strip markdown wrapping like ```json
    langflowOutput = langflowOutput
      .replace(/^```json/, "")
      .replace(/^```/, "")
      .replace(/```$/, "")
      .trim();

    // Clean smart quotes (if any)
    langflowOutput = langflowOutput.replace(/[\u201C\u201D]/g, '"');

    let parsed;
    try {
      parsed = JSON.parse(langflowOutput);
    } catch (parseError) {
      console.error("❌ JSON parse failed. Output was:", langflowOutput);
      throw new Error("Langflow returned invalid JSON format.");
    }

    return {
      score: parsed.resume_match_score,
